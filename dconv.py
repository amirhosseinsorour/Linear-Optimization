...
class Conv(MovingCameraScene):
    ...
    def construct(self):
        ...
        # Introduction
        title = Tex("Discrete Convolution")
        title.scale(2.5)
        ...
        conv_text = Tex("Discrete Convolution:")
        ...
        # step 1: n -> k
        n_to_k = Tex("""$$Step \\ 1: \\; n \\rightarrow k$$""")
        n_to_k.scale(0.9)
        ...
        hk_to_hmk = Tex("""$Step \\ 2: \\; h[k] \\rightarrow h[-k]$""")
        hk_to_hmk.scale(0.9)
        ...
        wind_and_multiply = Tex("Step 3 \\ : Sliding Window")
        ...
        # writing h[%d - k]
        h_dmk = Tex("{\\footnotesize$h[ %d -k]$}" % (2 * offset))
        h_dmk.set_color(BLUE)
        h_dmk.move_to([3.5, 2.3, 0])

        # n counter
        n_counter = Tex("{n = \\footnotesize$ %d $}" % (2 * offset))
        ...
        for i in range(1, int(2 * (right_offset - offset)) + 1):
            next_h_nmk = Tex("{\\footnotesize$h[ %d -k]$}" % (2 * offset + i))
            next_h_nmk.set_color(BLUE)
            next_h_nmk.move_to([3.5, 2.3, 0])
            next_n_counter = Tex("{n = \\footnotesize$%d$}" % (2 * offset + i))
            ...
            for j in range(len(th)):
                ...
                if 1 < i < 5:
                    ...
                    h_term = Tex('{:.2f}'.format(yh[len(th) - j - 1]))
                    mul_symbol = Tex('×')
                    x_term = Tex('{:.2f}'.format(yx[i + tw_min + j]))
